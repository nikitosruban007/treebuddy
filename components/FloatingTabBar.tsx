...
export default function FloatingTabBar({
  ...
  const activeTabIndex = React.useMemo(() => {
    ...
    tabs.forEach((tab, index) => {
      ...
      const routeString = typeof tab.route === 'string'
        ? tab.route
        : (tab.route?.pathname || '');

      // Exact route match gets highest score
      if (pathname === routeString) {
        score = 100;
      }
      // Check if pathname starts with tab route (for nested routes)
      else if (routeString && pathname.startsWith(routeString)) {
        score = 80;
      }
      ...
      else if (routeString.includes('/(tabs)/')) {
        const afterTabs = routeString.split('/(tabs)/')[1];
        if (afterTabs && pathname.includes(afterTabs)) {
          score = 40;
        }
      }
